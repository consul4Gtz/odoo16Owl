...
class L10nHNSar(models.Model):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        credit_note = options.get('move_type', 'out_refund')
        date_from = options['date']['date_from']
        date_to = options['date']['date_to']
        empresa = options.get('selected_partner_ids')
        #company_ids = options.get('company_id')
        ...
        lines = []
        line_id = 0
        sign = 1.0 if credit_note == 'out_refund' else 1.0

        totals = {}.fromkeys(['gravado', 'gravado15', 'gravado18', 'exento', 'exonerado', 'impuesto15', 'impuesto18',
                              'total'], 0.0)
        ...
        if journal != 'Todos los diarios':
            journal_list = journal.split(',')
            journal_list = [line.strip() for line in journal_list]
            domain += [('journal_id.code', 'in', journal_list)]
        ...
        for rec in self.env['account.report.sar'].search_read(domain):
            ...
            lines.append({
                ...
                'columns': [
                    ...
                    {'name': formatLang(self.env, sign * rec['exento'], currency_obj=self.env.company.currency_id)},
        ...
        lines.append({
            ...
            'columns': [
                ...
                {'name': formatLang(self.env, totals_rounded['exento'], currency_obj=self.env.company.currency_id)},
