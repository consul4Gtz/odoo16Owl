...
class AccountWithholding(models.Model):
    ...
    name = fields.Char('Number', required=True,  copy=False, default='/')
    ...
    account_date = fields.Date(index=True, )
    ...
    journal_id = fields.Many2one('account.journal', domain=lambda self: [('company_id', '=', self.env.company.id)],default=_get_default_journal)
    ...
    wtax_tamount = fields.Monetary('Withholding Total', currency_field='currency_id', store=True, 
...
class AccountWithholdingLine(models.Model):
    ...
    withholding_id = fields.Many2one('account.withholding', index=True, required=True,  auto_join=True,
    ...
    account_date = fields.Date(related='withholding_id.account_date', index=True,  store=True,)
    ...
    wtax_amount = fields.Monetary('Monto Retención', currency_field='currency_id', )
    ...
    def _prepare_wtholding_moves(self):
        rec = self
        ref = f'{rec.wtax_id.name} - {rec.invoice_id.name}'
        company_currency = rec.company_id.currency_id  # Definición de company_currency
    
        counterpart_amount = rec.wtax_amount
        # Verifica si rec.currency_id es igual a company_currency
        ...
        move_vals = {
            ...
            'line_ids': [
                (0, 0, {
                    'name': ref,
                    'amount_currency': counterpart_amount if rec.currency_id else 0.0,
                    'currency_id': rec.currency_id.id,
